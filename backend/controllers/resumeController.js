...
export async function analyzeResumeController(req, res) {
  ...
  try {
    ...
    const resumeAnalysis = await analyzeResume(resumeText, githubData, targetRole || "fullstack");
    ...
    return res.status(500).json({ message: "Failed to analyze resume: " + err.message });
  }
}
